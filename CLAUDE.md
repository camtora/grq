...
chat. Decision sessions run on **Opus 5.5** (`claude-opus-5-5`, effort pinned `high` — D128; was Opus 4.8), triage on Haiku 4.5 — Fable 5
...
shows the active brief (a blank ↻ refresh / the daily 8:00 hunt goes broad again — moved pre-open in D98 so it
drains the single session lane before the 10:00 check-in instead of colliding with it). Reach is **North America
...
1. **Hard guardrails live in code and only humans change them** (`PROJECT_PLAN.md` §6).
   ...
   **D126 (2026-09-22):** the agent can now also **demote** (ACTIVE → CANDIDATE) to free its own
   slot under the 60-name cap — promote had no counterpart, so the universe filled at 25/wk and
...
#   check-in (CHECKIN_TIMES_ET — every-30-min slots 10:00–15:30 ET since D96) UNLESS explicitly told to — a restart kills
...
| `web/app/options/` · `web/lib/options/{payoff,greeks,strategies,probability}.ts` · `web/components/options/` · `web/app/api/options/chain/[symbol]` | **Options education portal (D100) — top-level `/options` hub, EDUCATION-ONLY (modeled, never executable; touches no gate/broker/fund).** Tabs Learn · Calculator · Experiment · Ask. The calculator is an optionsprofitcalculator-style payoff tool: pure cents engines (`payoff`, `greeks`, `probability` — greeks+prob are new; reuses `price.ts` BS + the CBOE chain), `PayoffChart` (interactive SVG), a price×date P/L heat table + prob-of-profit, live-chain prefill. Experiment tab = `loadDesk()` contracts + value-over-time + "load into calculator"; chat is options-aware via the read-only `get_options_desk` tool. 4 v1 strategies (long call/put, covered call, cash-secured put). Distinct from the Options Desk (D91) + Alfred Options (D99). `docs/OPTIONS-PORTAL.md` |
| `web/app/learn/` · `web/lib/learn/content.ts` · `web/components/learn/` | **The Learn portal (D110) — top-level `/learn` beside Reports.** Teaches how the market WORKS (not stock picks): 8-course curriculum (1–3 live: the machine · how a price happens · owning a piece; Options = the D100 portal as Course 6, unmoved), labs rail, `/learn/glossary` (searchable `GLOSSARY` browser), Ask via `grq:chat`. Lessons = markdown-as-data rendered by `Md` (`[[term]]`→`<Term>`); web owns content (build context = ./web, can't import `shared/`) — `scripts/export-learn-content.ts` mirrors to `shared/content/learn.json` for GRQ Go; `test/learn-content.test.ts` enforces no dead term refs/links. Design: `docs/LEARN-PORTAL.md` |
...
| `web/agent/council.ts` | **The LLM Council (D115)** — a five-lens advisory panel: Opus argues a question as Contrarian · First-Principles · Expansionist · Outsider · Executor (parallel), then a Chairman synthesizes ONE verdict. **ADVISORY ONLY** — text, no order path, never the §6 gate. Two uses: (1) chat **HARD-GATES** stock-judgment Qs through it (a Haiku router `routeChatToCouncil` decides; fails open to Alfred), (2) the decision session is **prompted** to `convene_council` before every BUY / close-call SELL. Six Opus passes/convene on the Max token → kill without a deploy: `GRQ_COUNCIL_ENABLED=false`. Lives in the agent+chat images only (imports the SDK) |
| `web/agent/limit-quiet.ts` · `web/agent/retry.ts` | **Claude-limit quiet (D123, 2026-09-11).** A usage/spend/rate-limit error from ANY Claude call (`runSession`, council `oneShot`, chat, explain) arms a DB-durable quiet (`AgentState.limitQuietUntil`): every Claude session is skipped — no call, no per-session alert — until the reset the error names, else the next 5h-window reset rolled from the /tokens anchor (`Settings.maxWindowResetAt` — keep it honest, a 5h grid drifts), else +60 min. **One ping per ET day.** **D124:** the ACCESS family (org disabled subscription access / dead token) trips the same quiet but the ping says a human must act; and `alert()` **holds any warning/critical that repeats verbatim within 30 min** (journal still written) — the class fix for every retry-every-tick storm. `attemptGate` stops the race/desk retry storm (a run whose entrants all fail is "due" every 60s tick otherwise). Lift early: `update "AgentState" set "limitQuietUntil"=null where id=1`. Never touches §6 / kill switch / reconcile / OpenRouter |
| `web/agent/` | The agent worker (Phase 2): `runner.ts` (orchestrator/tick loop), `validator.ts` (§6 gate), `policy.ts` (hard limits + model IDs + `COUNCIL`), `sessions.ts` (LLM sessions), `tools.ts` (incl. `convene_council`), `context.ts`, `signals.ts`, `calendar.ts`, `alerts.ts`, `chat-server.ts` (the chat council hard gate), `demote.ts` (**D126** — the agent's slot-reclaim: `decideDemote` is pure + tested; demote-only, never retire). **D35:** scheduled intraday check-ins (`CHECKIN_TIMES_ET` — every 30 min 10:00–15:30 ET since D96, with 25-min fire windows + the daily hunt moved to 8am to keep the lane free, D98; noon is the midday BRIEF not a check-in; day bookended by the 9:00 morning plan + 16:15 EOD brief) + agent self-scheduling (`AgentWakeup` + `schedule_checkin`/`list_scheduled`/`cancel_checkin`). **D37:** conviction tally (`TradeProposal`, logged at `propose_order`) + durable lesson banking. **D39:** active-deployment mandate — PERSONA flipped to "put the fund to work" (month-over-month scorecard, under-deployment = the failure mode), morning "WIDEN IF THIN" hunt, check-ins treat the plan as a revisable hypothesis, `SELF_INVEST.maxPerRollingWeek` 2→5→25. The §6 gate + conviction bar were UNCHANGED by D39 — the fix was disposition + breadth, not a lower bar (the bar itself later moved 75→70, D95). **D40:** daily cadence — hourly check-ins 10/11/13/14/15, noon midday brief, 9:00 plan + 16:15 EOD bookends |
...
| `mobile/` | **GRQ Go (D106)** — the Expo/RN app, THE mobile client (the old SwiftUI `ios/` natives were DELETED 2026-07-04 at full parity; their history + docs/IOS-*.md remain the record). Metro via `scripts/grq-metro.sh` (port 8082), Mac build via `scripts/grqgo-mac-sync.sh`. Runbook: `docs/MOBILE-GRQGO.md` |
...
| `.env` | Secrets/config: db password, `BROKER=ibkr-paper`, `CLAUDE_CODE_OAUTH_TOKEN` (Cam's Max token), `DISCORD_WEBHOOK_URL` (alerts), `FMP_API_KEY`, optional `GRQ_MODEL_DECISION` (default `claude-opus-5-5`) + `GRQ_EFFORT_DECISION` (default `high`, D128); **`GITHUB_TOKEN` (D82, daily build diary):** a fine-grained read-only PAT (Contents:Read on `camtora/grq`, env_file rule → unquoted) the 3am `runDailyChangeReport()` uses to read the day's commits for Graham's plain-English changelog (`lib/github.ts`; `GITHUB_REPO` defaults camtora/grq). **`GITHUB_BRANCH` is load-bearing — the commits API defaults to the repo's DEFAULT branch (main), which is once again the active dev line (`feat/the-race` was merged to `main` and retired 2026-07-18), so `GITHUB_BRANCH=main`; if dev ever moves back to a feature branch, set it to that branch or the diary comes up empty, and update it when the long-lived branch changes; **IBKR (D22, D33):** `IBEAM_ACCOUNT`/`IBEAM_PASSWORD` (the PAPER login `yzfrmq515` — UNQUOTED, env_file rule), `IBEAM_USE_PAPER_ACCOUNT=True`, `IBKR_ACCOUNT_ID=DUQ779121`, `IBKR_GATEWAY_URL=https://ibeam:5002` (the loopback proxy, not :5000); **iOS push (D53, LIVE-to-production 2026-06-24):** `APNS_KEY_ID`/`APNS_TEAM_ID`/`APNS_BUNDLE_ID`/`APNS_KEY_B64` (.p8 base64 — env_file rule: unquoted). **⚠️ TWO env-split `.p8` keys exist (repo root, gitignored): production push REQUIRES `APNS_KEY_ID=93LXUPS3V6`; `9VAQ4T6CYS` is sandbox-only and silently drops every TestFlight device** (the wall that kept push dark — wrong-key returns `403 BadEnvironmentKeyInToken`, not an obvious error). **Env-only change → NO rebuild: `docker-compose up -d --force-recreate web agent chat`.** Verified end-to-end to Cam's TestFlight phone 2026-06-24. `pushNotify` no-ops per-recipient when a member has no `DeviceToken` row (Graham hasn't opened the app). Full saga + diagnostics in `docs/PUSH-NOTIFICATIONS.md` (Troubleshooting). |
