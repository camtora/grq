...
| `mobile/` | **GRQ Go (D106)** — the Expo/RN rebuild of the iOS app (third iteration; `ios/` natives retired-in-place). Splash port only + placeholder tabs so far; same mobile backend. Metro via `scripts/grq-metro.sh` (port 8082), Mac build via `scripts/grqgo-mac-sync.sh`. Runbook: `docs/MOBILE-GRQGO.md` |
