...
type Tab = "messages" | "agent";

/** Slide-out chat drawer (right side, ~1/3 width, overlays content). Two tabs:
 *  Messages (the Cam↔Graham direct thread) and Ask GRQ (the read-only agent).
 *  Open it from anywhere:
 *    window.dispatchEvent(new CustomEvent("grq:chat",
 *      { detail: { symbol?: "RY", tab?: "messages" | "agent" } }))
 *  A detail.symbol opens the Agent tab to discuss that stock; detail.tab forces a
 *  tab. Escape or ✕ closes. Stays mounted after first open so both threads survive
 *  open/close. */
export default function ChatDrawer({
  ...
  // Agent tab: your thread first, then the other member's — toggle between them.
  ...
  useEffect(() => {
    function onChat(e: Event) {
      const detail = (e as CustomEvent).detail as { symbol?: string; tab?: Tab } | undefined;
      if (detail?.symbol) {
        setSymbol(detail.symbol.toUpperCase());
        setTab("agent"); // discussing a stock → the agent
      } else if (detail?.tab) {
        setTab(detail.tab);
      }
      setOpen(true);
  ...
  const tabBtn = (t: Tab, label: string) => (
    <button
      ...
      className={`rounded-md px-3 py-1 text-xs font-semibold transition-colors ${
        tab === t ? "bg-teal-400/20 text-teal-100" : "text-teal-200/50 hover:bg-teal-400/10"
      }`}
    >
      {label}
    </button>
  ...
  return (
    ...
    >
      ...
      {/* Agent sub-controls: whose agent thread + the read-only reminder. */}
      {tab === "agent" && (
        <div className="flex items-center gap-3 border-b border-teal-400/10 px-5 py-2">
          {threads.length > 1 && (
            <div className="flex items-center gap-0.5 rounded-lg border border-teal-400/15 p-0.5">
              {threads.map((m) => (
                <button
      ...
      <div className="min-h-0 flex-1 px-5 py-4">
        {everOpened && (
          <>
            {/* Both panes stay mounted; we just hide the inactive one so each thread
                keeps its scroll + draft. */}
            <div className={tab === "messages" ? "h-full" : "hidden"}>
              <MemberChat active={open && tab === "messages"} heightClass="h-full" />
            </div>
