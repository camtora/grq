...
| 2026-06-22 (Cam) | **The Wire — "the Hunt meets Instagram"** — a full-screen, vertically-paged discovery feed (iOS-first) that weaves hunt finds, fresh dossiers, the watchlist board (Cam/Graham/agent), market news, and glossary lessons into one swipeable stream — five purpose-built full-screen card layouts. **v1 shipped 2026-06-22** — shared + read-only, no schema change; `GET /api/wire`. **Phase 2:** per-user interests/saved briefs (→ "for you"), price alerts + push (wired to the D53 stack), stock-tied articles, web rendering, unpriced-finds coverage. Full plan + requirements: `docs/THE-WIRE.md`; decision record: `docs/DECISIONS.md` D55. |
