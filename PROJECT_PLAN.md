...
**Initial contribution:** $25,000 CAD (Cam's money; a learning project for both) — bumped from $5,000 (D31)
**Status:** Phases 0–2.7 shipped — agent **live-fire on IBKR paper since 2026-06-17** (`BROKER=ibkr-paper`; IBKR-paper soak clock day 1 = 2026-06-17; sim soaked 2026-06-12→17), plus backups, per-member themes, stocks one-pagers, signals v1, source scoreboard, member directives, UI-managed universe + research dossiers, read-only viewer tier, literacy explainers, and the read-only agent chat. **Data layer now live on FMP Ultimate + free BoC feeds (D21):** tiers 2/5/6/7 (fundamentals/13F/earnings/news + analyst grades) + tier 9 macro (Bank of Canada + **US via FRED**) — surfaced on the stock pages (with an honest 10-tier coverage map) AND fed into the agent's decision context; tier 4 insider **structured for US + cross-listed CA** (FMP Form 4 + OpenInsider); pure-TSX via agent web-research (free CA sources walled). **Real-time on-page price ticker** via FMP (`<LiveQuote>`; verify TSX-realtime-vs-delayed at market open). **Rating: GRQ's call unified to a 7-point scale** (Strong Buy→Strong Sell, same vocabulary as the signal; D23). **IA-v3 (D23):** Market lands on **Watchlist** (primary), **Universe** is a background sub-tab, "Ideas"→**Discoveries**, **Research** = your human notes desk, Watchlist rows condensed→expand, Today/Brief = news-on-top + industry breakdown + full-width game plan, stock search does name + multi-listing. **Phase 3 (IBKR paper) — LIVE (D22, D33):** `BROKER=ibkr-paper` and the agent trades the paper account **`DUQ779121`** (CAD ~25k; the 2026-06-17 reset re-provisioned it — was `DUQ774890`/5k) via a **loopback proxy** (`grq-ibeam-proxy` socat sidecar → `IBKR_GATEWAY_URL=https://ibeam:5002`). **Verified end-to-end 2026-06-17 (D33):** connect + `reconcile()` + a 1-share XIC market order placed→filled @ CAD 56.98→reconciled (the "No trading permissions" block cleared on the overnight perm-sync); D33 also fixed a slow-fill ledger gap (`finalizePending()`). **The ≥2-wk IBKR-paper soak clock started 2026-06-17 = day 1.** Gateway needs a daily ~midnight-ET IB Key re-approval. **NB SPCX = the SpaceX *CDR* (`SPCX.TO`, CAD-hedged ~$36), not the Nasdaq underlying. The fund now trades CAD **and** USD (D34 — multi-currency mirror of IBKR, live 2026-06-17).**
...
| **3 — Paper trading** ✅ **LIVE 2026-06-17 (D33), soaking** | IBeam + `IBKRBroker` adapter on the **paper account `DUQ779121`**; flip `BROKER=ibkr-paper` (done); Flex import deferred | **Yes** | **≥ 2 clean weeks on IBKR paper, ≥ 4 clean weeks total incl. sim** (defined below) — paper day 1 = 2026-06-17 | 2–3 days + soak |
...
| Added | Item |
|---|---|
| 2026-09-22 (Cam — OPEN, not approved) | **Universe cap 60 → 80?** Recommended after D126/D126a; Cam understood the reasoning and did **not** decide, so the cap stays **60**. The argument: ACTIVE only means *eligible if it triggers*; the real anti-runaway bounds are the weekly BUY cap, position sizing and the cash floor. 60 was picked 2026-06-17 to never bind (2 promotes/wk = ~30 weeks of headroom) and D39 took the inflow to 25/wk the next day without revisiting it. Observed: the fund wants ~62–65 eligible names, so every new idea pays a prune-first tax. **Caveat added by D126b:** the new daily-refresh gate exempts ACTIVE names, so raising the cap is no longer token-neutral — do the ACTIVE-gate item below first if cost matters. Humans-only (rule #1) — ask, don't assume. |
| 2026-09-22 (D126b follow-up) | **Extend the daily-refresh gate to ACTIVE names** — `decideDailyRefresh` currently exempts everything that isn't a CANDIDATE, so **37 of 62 ACTIVE names are not buy-rated yet still get a daily dossier** (~8/day, ~2.6M tokens). Gating unheld + unwatched + non-buy ACTIVE names the same way would recover that and make the universe cap token-neutral again. Deliberately left out of D126b: ACTIVE is the tradeable, money-adjacent set and the slice is small next to the ~17M/day the candidate gate already saved. |
| 2026-09-22 (found while shipping D126b) | **`tsconfig.json` excludes `test`** — test files are never typechecked, so adding a required field to a decision type does not fail the build from the test side. Caught live: the prune tests ran happily with `nonBuyStreak: undefined`, silently never exercising the new rule. Tests still *run* (tsx is loose at runtime); they just aren't type-checked. Small fix, real blind spot. |
| 2026-09-22 (found while shipping D126) | **`test/auth-jwt.test.ts` is a time-bomb and has been failing since ~2026-09-15** — `NOW = 1_789_000_000` is a fixed constant (~2026-09-10) but `jwt.verify` uses the real clock, so the 25-day-old token it mints is now genuinely expired and `refreshGrqToken` returns null. The **D125 sliding-session behaviour has been unguarded for a week**. One-line fix (`clockTimestamp: NOW`, or verify with `ignoreExpiration` in the helper). Pre-existing; unrelated to D126. |
...
| 2026-06-22 (Cam) | **The Wire — "the Hunt meets Instagram"** — a full-screen, vertically-paged discovery feed (iOS-first) that weaves hunt finds, fresh dossiers, the watchlist board (Cam/Graham/agent), market news, and glossary lessons into one swipeable stream — five purpose-built full-screen card layouts. **v1 shipped 2026-06-22** — shared + read-only, no schema change; `GET /api/wire`. **Phase 2 part 1 shipped 2026-06-22 (D56):** **price alerts + push** (`PriceAlert` table + `/api/notifications/price-alerts` + runner `checkPriceAlerts()` → owner-only push, category `priceTargets`; iOS bell + manager), **stock-tied articles** (`fmpStockNews` → `relatedTickers` chips), **lesson richness** (glossary `example`/`related` → example line + tappable terms). **Phase 2 part 2 shipped 2026-06-22 (D57) — the Wire goes social:** viewer-aware watch lane (hides your own watches, shows the other member's), richer watch cards (full dossier substance) + richer find cards (absolute targets + full thesis), and **all 55 glossary terms enriched** (the lessons "looked unchanged" because D56 enriched only ~18 and the 3/day rotation kept missing them). **Phase 2 remaining:** deeper per-user "for you" ranking (interests/saved briefs), web rendering, unpriced-finds coverage. Full plan + requirements: `docs/THE-WIRE.md`; decision record: `docs/DECISIONS.md` D55 + D56 + D57. |
| 2026-06-26 (Cam) | **News & events layer** — *planned (D81); `docs/NEWS-AND-EVENTS.md`.* Stop throwing FMP news away and stop flattening macro to a one-line level: one persisted, **Haiku-triaged** event store fanned out three ways — agent "what moved" digest, the existing human pages (Today/The Wire/stock-page news, which stop live-fetching), and a **news-driven wakeup** (agent reacts only to price + the clock today). Capture (deterministic, ~free) → Triage (Haiku, pennies) → Serve (Opus sees only the digest — off the Max quota). News + events are an INPUT the agent weighs, never the gate (§6 untouched). Two additive tables (`NewsArticle`, `MarketEvent`); M1 = macro→events, M2 = news. The **knowledge graph** is the explicit follow-on (events are what a `CompanyEdge` set would propagate). |
| 2026-06-22 (Cam) | **Owner tier + admin usage dashboard** — **shipped 2026-06-22 (D58).** A third auth tier ABOVE member (`isOwner`, default Cam, `OWNER_EMAILS` env) gates `/admin`, an owner-only usage dashboard. Traffic logging via a client beacon (`Tracker` → `POST /api/track`, identity resolved server-side) into a `PageView` table; the dashboard aggregates views per person × section over 24h/7d/30d/90d. NB the existing `GrqUser.role:"admin"` field is cosmetic — `isOwner` is the real gate. Decision record: `docs/DECISIONS.md` D58. |
