...
| `SectionHeader` | `<h2 className="text-base font-bold uppercase tracking-wide text-teal-100">` + optional `sub` (trailing `text-xs font-normal normal-case text-teal-200/45` descriptor) + `right` slot, wrapped `mb-3 flex items-end justify-between`. `size="lg"` bumps title→`text-lg` / sub→`text-sm` — the Today newspaper runs `lg` (Cam 2026-07-03), everything else the `md` default | top-level **page sections** (Today `lg`, Portfolio `md`) |
